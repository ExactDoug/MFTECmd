...
5. **WSL interop hiccups.** `UtilAcceptVsock:273: accept4 failed 110` is transient; retry the
   command before diagnosing anything.
...
12. **`find -maxdepth 4` misses the fixtures.** They are 5 levels below `~/dev`. That is why
    an early search concluded no `$MFT` existed.
...
15. **The same two research reports exist in both repos** (mftecmd `docs/`, ntfsight
    `docs/research/`). Change both or neither.
16. **Untracked irreplaceable data lives in ntfsight.** Never `git clean -fdx` there (item 09).
